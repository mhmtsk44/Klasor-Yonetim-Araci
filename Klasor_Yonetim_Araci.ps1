...
# -------------------------------------------------------------------------
# YÖNETİCİ (ADMIN) KONTROLÜ VE UZAKTAN ÇALIŞTIRMA YEDEĞİ
# -------------------------------------------------------------------------
function Test-Admin {
...
if (-not (Test-Admin)) {
    ...
    try {
        if (-not [string]::IsNullOrWhiteSpace($BetikYolu) -and (Test-Path $BetikYolu)) {
            ...
            $argList = "-NoProfile -ExecutionPolicy Bypass -NoExit -File `"$BetikYolu`""
        ...
        [System.Windows.Forms.MessageBox]::Show("Yönetici izni alınırken hata oluştu veya işlem reddedildi.`n`nAyrıntı: $($_.Exception.Message)", "Yönetici İzni Gerekli", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
    exit
}
